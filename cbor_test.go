...
func HexEncode(src []byte) string {
	var d string
	for i, v := range src {
		d += string(hextable[v>>4])
		d += string(hextable[v&0x0f])
		if v >= 32 && v < 127 {
			d += "(" + string(v) + ")"
		}
		if i%16 == 15 {
...
func DecodeRequiredNullable(decoder *cbor.Decoder) (*Required, error) {
...
func DecodeRequired(decoder *cbor.Decoder) (Required, error) {
	var o Required
	err := o.Decode(decoder)
	return o, err
}

func (o *Required) Decode(decoder *cbor.Decoder) error {
	numFields, _, err := decoder.ReadMapSize()
	if err != nil {
		return err
	}
		...
		case "arrayValue":
			...
			if err == nil {
				if isNil {
					...
					size, _, err := decoder.ReadArraySize()
					if err == nil {
		...
		case "mapValue":
			...
			if err == nil {
				if isNil {
					...
					size, _, err := decoder.ReadMapSize()
					if err == nil {
...
func (o *Required) Encode(encoder cbor.Writer) {
	if o == nil {
		encoder.WriteNil()
		return
	}
